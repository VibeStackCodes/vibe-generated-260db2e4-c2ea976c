...
Make authentic European products effortlessly discoverable and shippable across borders by combining curated vendors, local‑native shopping experiences, and transparent cross‑border checkout for shoppers and a trusted growth channel for small European merchants.

- Localized storefronts: language, currency, localized delivery options and UX per market
- Transparent pricing: upfront VAT, duties and shipping estimator shown before checkout
- Product catalog: category browse, filters (price, brand, rating), PDPs with 3+ images, variants, stock status
- Search: keyword search with autocomplete and relevance sorting (post-MVP: faceted & typo tolerance)
- Shopping cart & guest checkout: add/remove, promo codes, shipping cost estimator
- Secure checkout & payments: Stripe + PayPal (PCI tokenization), HTTPS; roadmap for local pay methods (Klarna, Apple/Google Pay)
- Basic customer accounts: signup/login, order history, address book
- Admin dashboard (MVP): product management (CSV upload), order management, basic reporting (orders, revenue)
- Fulfillment & returns (MVP integration): 3PL/partnered regional hubs for EU-wide fulfillment and simple returns flow
- Trust & curation: seller verification workflow, curated onboarding, sustainability packaging options (displayed on PDP)
- Marketing & personalization (post-MVP): promo engine, rule-based recommendations, collections, subscriptions
- Accessibility & responsive UI: mobile-first design, WCAG AA basics
- Observability & ana
